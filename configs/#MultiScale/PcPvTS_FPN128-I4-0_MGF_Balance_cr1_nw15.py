# 1. 模型配置(models) =========================================
model = dict(
    backbone=dict(type="PcPvT-S", pretrain=True, output_index=[0, 1, 2]),
    neck=dict(
        type="FPN_I3",
        output_dims=128,
        UAV_output_index=[0],
...
# 3. 训练策略配置(schedules) =========================================
lr_config = dict(lr=1.5e-4, type="cosine", warmup_iters=0, warmup_ratio=0.01)
train_config = dict(
    autocast=True,
    num_epochs=12,
)
