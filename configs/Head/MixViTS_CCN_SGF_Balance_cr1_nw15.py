...
model = dict(
    backbone=dict(
        type="MixFormer",
        vit_type="small_patch16",
        pretrain_path="pretrain_model/timm_vit_small_patch16_384.pth",
        pretrain=True,
        output_index=[0],
    ),
    neck=dict(
...
# 3. 训练策略配置(schedules) =========================================
lr_config = dict(lr=5e-5, type="cosine", warmup_iters=500, warmup_ratio=0.02)
train_config = dict(autocast=True, num_epochs=48)
test_config = dict(
    num_worker=8,
    filterR=1,
