...
model = dict(
    ...
    neck=dict(
        type="FPN",
        output_dims=128,
        UAV_output_index=[0, 1, 2, 3],
        Satellite_ouput_index=0,
    ),
    head=dict(
        type="MultiEnhanceGroupFusionHead",
        head_pool="avg",
        gc_mid_channel=16,
        single_output_channel=8,  # single_output_channel = neck["output_dims"]/gc_mid_channel
        merged_linear_layer_num=1,
        merged_linear_channel=64,
    ),
    loss=dict(
        cls_loss=dict(type="BalanceLoss", center_R=7, neg_weight=15),
        # reg_loss=dict(
        # )
    ),
)
...
train_config = dict(autocast=True, num_epochs=16)
test_config = dict(
    num_worker=8,
    filterR=1,
