...
model = dict(
    backbone=dict(
        type="ViT-S",
        pretrain=True,
        pretrain_path="/home/dmmm/VscodeProject/FPI/pretrain_model/denseUAV_ViTS_patch16_timm.pth",
        output_index=[0],
    ),
    neck=dict(
        type="CCN",
...
# 3. 训练策略配置(schedules) =========================================
lr_config = dict(
    lr=1.5e-4, type="cosine", warmup_iters=500, warmup_ratio=0.1, backbone_lr_rate=1.0
)
